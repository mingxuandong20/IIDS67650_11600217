...
train_surv <- with(train_data, Surv(start, stop, CVD))

###Build the Cox model
cox_model <- coxph(train_surv ~ AGE + SEX + SYSBP + BMI + TOTCHOL + CURSMOKE + DIABETES, data = train_data,
                   x = TRUE,
                   y = TRUE)
...
plotROC(sc_cox_td, times = times_days[1], col = cols[1], lwd = 2,
        main = "Time-dependent ROC of COX model")
plotROC(sc_cox_td, times = times_days[2], add = TRUE, col = cols[2], lwd = 2)
...
plotCalibration(sc_cox_td, times = times_days[1], legend = FALSE,
                xlab = "Predicted risk @5y (cox)", ylab = "Observed risk @5y", lwd = 2)

plotCalibration(sc_cox_td, times = times_days[2], legend = FALSE,
                xlab = "Predicted risk @15y (cox)", ylab = "Observed risk @15y", lwd = 2)

#####################################
##Cox model with different missing ratios
###0.2
cox_model1 <- coxph(Surv(start, stop, CVD) ~ AGE + SEX + SYSBP + BMI + TOTCHOL + CURSMOKE + DIABETES, data = train_data_mcar1,
                    x = TRUE,
                    y = TRUE)
summary(cox_model1)
#exp(coef(cox_model1))
...
###0.4
cox_model2 <- coxph(Surv(start, stop, CVD) ~ AGE + SEX + SYSBP + BMI + TOTCHOL + CURSMOKE + DIABETES, data = train_data_mcar2,
                    x = TRUE,
                    y = TRUE)
summary(cox_model2)
#exp(coef(cox_model2))
...
###0.6
cox_model3 <- coxph(Surv(start, stop, CVD) ~ AGE + SEX + SYSBP + BMI + TOTCHOL + CURSMOKE + DIABETES, data = train_data_mcar3,
                    x = TRUE,
                    y = TRUE)
summary(cox_model3)
#exp(coef(cox_model3))
...
###0.8
cox_model4 <- coxph(Surv(start, stop, CVD) ~ AGE + SEX + SYSBP + BMI + TOTCHOL + CURSMOKE + DIABETES, data = train_data_mcar4,
                    x = TRUE,
                    y = TRUE)
summary(cox_model4)
#exp(coef(cox_model4))
...
###Univariate condition is missing
###BMI missing 60% in smokers
cox_model_mar1 <- coxph(Surv(start, stop, CVD) ~ AGE + SEX + SYSBP + BMI + TOTCHOL + CURSMOKE + DIABETES, data = train_data_mar1,
                        x = TRUE,
                        y = TRUE)
summary(cox_model_mar1)


###MAR  Multivariate MAR
cox_model_mar2 <- coxph(Surv(start, stop, CVD) ~ AGE + SEX + SYSBP + BMI + TOTCHOL + CURSMOKE + DIABETES, data = train_data_mar2,
                        x = TRUE,
                        y = TRUE)
summary(cox_model_mar2)

###calibration
preds_mar <- list(
  "MAR-Smoke60"     = cox_model_mar1,
  "MAR-Older&DM60"  = cox_model_mar2
)
