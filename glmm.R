...
glmm_model <- glmer(
  CVD ~ AGE + SEX + SYSBP + BMI + TOTCHOL + CURSMOKE + DIABETES + (1 | RANDID),
...
#Test set prediction probability (population mean, excluding random effects)
glmm_pred <- predict(glmm_model, newdata = test_data, type = "response", re.form = NA)

#The guaranteed label is a value of 0/1
...
#Time-dependent assessment that is fully consistent with logistic regression
library(riskRegression)

sc_glmm_td <- Score(
  object   = list("GLMM" = glmm_pred),       
...
plotROC(sc_glmm_td, times = times_days[1], col = cols[1],  lwd = 2,
        main = "Time-dependent ROC - GLMM")
plotROC(sc_glmm_td, times = times_days[2], add = TRUE, col = cols[2],  lwd = 2)
legend("bottomright", legend = c("5-year ROC","15-year ROC"), col = cols, lwd = 2)

plotCalibration(sc_glmm_td, times = times_days[1], legend = FALSE,
                xlab = "Predicted risk @5y (GLMM)", ylab = "Observed risk @5y", lwd = 2)
plotCalibration(sc_glmm_td, times = times_days[2], legend = FALSE,
                xlab = "Predicted risk @15y (GLMM)", ylab = "Observed risk @15y", lwd = 2)
...
##Test set prediction (population mean, no random effects)
glmm_pred1 <- predict(glmm_model1, newdata = test_data, type = "response", re.form = NA)
...
summary(glmm_model2)
exp(fixef(glmm_model2))
glmm_pred2 <- predict(glmm_model2, newdata = test_data, type = "response", re.form = NA)
...
summary(glmm_model3)
exp(fixef(glmm_model3))
glmm_pred3 <- predict(glmm_model3, newdata = test_data, type = "response", re.form = NA)
...
summary(glmm_model4)
exp(fixef(glmm_model4))
glmm_pred4 <- predict(glmm_model4, newdata = test_data, type = "response", re.form = NA)
...
summary(glmm_mar1); exp(fixef(glmm_mar1))
p_mar1 <- predict(glmm_mar1, newdata = test_data, type = "response", re.form = NA)

#Absence of multivariate conditions: 60% absence of BMI for age >60 and diabetes
glmm_mar2 <- glmer(
  CVD ~ AGE + SEX + SYSBP + BMI + TOTCHOL + CURSMOKE + DIABETES + (1 | RANDID),
  family = binomial, data = train_data_mar2_cc,
  control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))
)
summary(glmm_mar2); exp(fixef(glmm_mar2))
p_mar2 <- predict(glmm_mar2, newdata = test_data, type = "response", re.form = NA)

###calibration
preds_mar <- list(
  "MAR-Smoke60"     = p_mar1,
  "MAR-Older&DM60"  = p_mar2
)
